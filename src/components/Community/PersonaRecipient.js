...
export function PersonaRecipient({personaId,passPersonaData}) {
  ...
  const [newPersonaId,updateNewPersonaId] = useState([])
  ...
  const format_array=(array)=>{
    ...
    updateNewPersonaId(mapping)
  }
  
  useEffect(() => {
  
   
    format_array(personaId)
...
<div onClick={() =>
{if (isSelected) {
  updateNewPersonaId((prev) => prev.filter((i) => i.id !== row.persona_id));
                                                } else {
                                                  let obj={
                                                    id:row.persona_id
                                                  }
                                                  updateNewPersonaId(prev => [...prev, obj])
                                                }}} >
     {personaId.map((item) =>
    {
