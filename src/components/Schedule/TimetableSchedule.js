...
export function TimetableSchedule({ filterText, onFilter }) {
  ...
  const [errorCodeSchedule, updateErrorCodeSchedule] = useState([]);
  ...
  console.log("//////////",publishDate,'/////',expireDate)
  ...
  async function fetchDepartmentWiseCourseList(e) {
    ...
    try {
      ...
      const fetchCourseResponse = await axios.post(
      ...
      // if (fetchCourseResponse.data.error_code == 404) {
      //   alert("Invalid Token OR Non Authorized User");
      //   window.location.href = "/";
      // }
    } catch (err) {
  ...
  async function fetchScheduleList() {
    if (
      ...
      history.push("/timetable", { departmentId, courseID, classID, section });
    }
  }
  ...
  return (
    <div className="content-wrapper">
      <div
        className="row"
        style={{ width: "100%", margin: "0", padding: "5px 30",background:"#ffffff",border:"1px solid #D9D9D9",alignItems:"baseline" }}
      >
        <div
          className="col-md-8"
          style={{ height: "100%", padding: "0px 0px 10px 37px" }}
        >
          <h4
            style={{
              color: "#000000",
      ...
      >
        <div
          class="ValueMsg"
          style={{ margin: "0px", width: "100%", display: "none" }}
        >
        ...
        <div className="row">
          ...
          {/* <div className="col-md-4" style={{paddingLeft:"0PX"}}>
      <label style={{
        fontFamily: "Poppins", fontStyle: "normal", fontWeight: "500", fontSize: "10px",
        lineHeight: "18px", color: "#1F3977"
      }}>Year</label><br></br>
 <input
                    type="text"
                    className="year_input_schedule"
                    id="faq_category_single"
                    placeholder="Add Year"
                    value={year}
                    onChange={(e) => updateYear(e.target.value)}
                    style={{ width: "100%", height: "38px",  fontSize: "11px", border: "1px solid #c4c4c4", background: "white" }}
